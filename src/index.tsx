...
app.post(
  ...
  async (request: Request, response: Response) => {
    const signature: string | undefined = request.headers[
      "stripe-signature"
    ] as string | undefined;
    ...
    switch (event.type) {
      case "checkout.session.completed":
        ...
        try {
          await pool.query(
            `INSERT INTO checkout_sessions (user_id, stripe_session_id, price_id, payment_status, payment_code, amount) VALUES ($1, $2, $3, 'completed', $4, $5)`,
            [
              userId,
              checkoutSession.id,
              priceId,
          ...
          if (fs.existsSync(avatarPath)) {
            ...
            base64ImageString = `data:${mimeType};base64,${fileData.toString(
              "base64"
            )}`;
          }
          ...
          const htmlString = ReactDOMServer.renderToStaticMarkup(
            <ComponentToPrint
              level={determineLevel(user.amount)}
              name={`${user.first_name} ${user.last_name}`}
              code={uniqueCodeGenerated}
...
function generateUniqueCode(): string {
  ...
  return segments
    .map((segment) => {
      return Array.from({ length: segment })
...
app.get("/donations", async (req: Request, res: Response) => {
  try {
    const result = await pool.query(
      "SELECT SUM(amount) AS total FROM checkout_sessions WHERE payment_status = $1",
      ["completed"]
    );
...
app.get(
  "/requestpdf/:checkoutSessionId",
  async (req: Request, res: Response) => {
    const checkoutSessionId = req.params.checkoutSessionId;

    const userQuery = await pool.query(
      `SELECT * FROM users LEFT JOIN checkout_sessions ON users.id = checkout_sessions.user_id WHERE checkout_sessions.stripe_session_id = $1`,
      [checkoutSessionId.replace(".pdf", "")]
    );
    const user = userQuery.rows[0];
...
app.get(
  "/requestpdfashtml/:checkoutSessionId",
  async (req: Request, res: Response) => {
    const checkoutSessionId = req.params.checkoutSessionId;
